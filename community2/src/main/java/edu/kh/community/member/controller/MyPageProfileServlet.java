...
public class MyPageProfileServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			...
			// --> multipartRequest (cos.jar 라이브러리 이용) 파일 업로드 라이브러리

			// ************** multipartRequest 사용을 위한 준비단계**************
			...
			// -> 변경된 파일명 ex) 20230330123054_01234.png
			...
			if(result > 0) { // 성공
				...
				session.setAttribute("message", "프로필 이미지 변경을 실패했습니다");
			}
			
			// 성공/실패 관계없이 프로필 화면 재요청(redirect)
			resp.sendRedirect("profile"); // 상대경로 (GET)   ,  
							//   └> myPage-profile.jsp 의 form 태크 action 값 
