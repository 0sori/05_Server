...
public class LoginServlet extends HttpServlet {
   ...
   protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
      ...
      try {
         ...
         // ** 로그인 **
         // ID / PW가 일치하는 회원 정보를 Session scope 에 세팅 하는 것
         ...
         if( loginMember != null ) { // 성공
            ...
            } else { // 체크 안했을때 (or 체크되어 있던걸 껐을 때)
               // 쿠키 파일을 0초 동안 유지
               // -> 기존에 존재하던 쿠키 파일의 유지 시간을 0초로 덮어씌움
               // == 삭제하겠다는 소리
               c.setMaxAge(0);
         ...
         //         클(라이언트)             캐셔(Servlet)         응답결과(JSP)
         
         // redirect(재요청)
         // - 클라이언트 재요청
         // - 현재 Servlet에서 응답 페이지를 만들지 않고
         //   응답 페이지를 만들 수 있는
         //   다른 요청의 주소로 클라이언트를 이용시키(재요청)
